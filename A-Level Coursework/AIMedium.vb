...
Public Class AIMedium
    ...
    Private Sub AIMedium_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "Player Snake"
    Private Sub SnakeMover_Tick(sender As Object, e As EventArgs)
        ...
        If gameend = True Then
            GlobalStuff.SortScore("AIMedium.txt")
            Me.Close()
        End If
    End Sub
    Private Sub AIMedium_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        SnakeMover.Start()
        AISnakeMover.Start()
...
#Region "AI Snake"
    Private Sub AISnakeMover_Tick(sender As Object, e As EventArgs)
        ...
        vector = (snake(0).Location - AIsnake(0).Location)

        'Add vector to AI snake current location
        AIsnake(0).Left += vector.X / 9
        AIsnake(0).Top += vector.Y / 9

        'Update status strip
        AI_location.Text = "AI Snake Location:(" & AIsnake(0).Top & "," & AIsnake(0).Left & ")"
    End Sub
#End Region
    Private Sub AIMedium_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        AISnakeMover.Stop()
        SnakeMover.Stop()
        eaten = Nothing
        body = Nothing
